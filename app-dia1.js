(() => {
  ...
  function missionContext(){
    ...
    const comicGalleryBlock=c.comicGallery?`<section class="d1-bloodbath-comic" aria-label="Ocho viñetas sobre el Baño de Sangre de Estocolmo">${c.comicGallery.map((item,i)=>`<figure><img src="${imgBase+item.file}" alt="${item.alt||item.name||"Viñeta histórica"}" ${i>0?"loading=\"lazy\"":""}><figcaption><strong>${item.name||`VIÑETA ${i+1}`}</strong>${item.caption?`<span>${item.caption}</span>`:""}</figcaption></figure>`).join("")}</section>`:"";
  ...
  function visual(kind,q,scope,solution=false){
    ...
    if(kind==="d3BalticTimeline")return d3BalticTimeline();
    ...
    if(kind==="d7PeatLayers")return d7PeatLayersCard();
    if(kind==="d7PeatTypes")return d7PeatTypesCard();
    if(kind==="d7PeatTimeline")return d7PeatTimelineCard();
    ...
    if(kind==="d9CrownRoad")return d9CrownRoadCard();
    if(kind==="d9Stures")return d9SturesCard();
  ...
  function d3BalticTimeline(){return `${d3BalticMap()}<div class="puzzle-card map-card baltic-timeline"><p class="kicker">CUATRO FECHAS · UN REY COMPARTIDO</p><div class="history-timeline"><div><strong>ANTES DE 1397</strong><span>Bogislav de Pomerania es adoptado por Margarita I y pasa a llamarse Erik.</span></div><div><strong>1397 · KALMAR</strong><span>Erik es coronado rey de Dinamarca, Noruega y Suecia: comparten monarca, no todas las leyes ni intereses.</span></div><div><strong>1412</strong><span>Muere Margarita I; Erik gobierna sin ella.</span></div><div><strong>1434</strong><span>La tensión política, comercial y fiscal llega a Bergslagen.</span></div></div><p class="map-caption">Una corona común no borraba las necesidades distintas de cada reino.</p></div>`}
  ...
  function d7ImageCard(kicker,file,alt,caption,note){return d7Card(kicker,`<figure class="visual-evidence peat-figure d8-mobile-figure"><img src="${imgBase}${file}" alt="${alt}" decoding="sync"><figcaption>${caption}</figcaption></figure>`,note)}
  function d7PeatCard(){return d7ImageCard("CÓMO NACE UNA TURBERA","komosse-01-lago-a-turbera-mobile-v3.jpg","Secuencia vertical desde el retiro del hielo hasta un humedal con turba","Un lago no desaparece de golpe: se va llenando de plantas y restos vegetales hasta transformarse en humedal.","La foto de Komosse muestra el paisaje actual; este esquema vertical explica los miles de años que hay debajo de los musgos.")}
  function d7PeatLayersCard(){return d7ImageCard("POR QUÉ LAS PLANTAS SE QUEDAN","komosse-02-capas-de-turba-mobile-v3.jpg","Corte vertical de una turbera con plantas vivas, restos vegetales y turba","En un suelo corriente hay aire y los restos se descomponen pronto. En la turbera, el agua deja poco oxígeno y parte de la vegetación queda acumulada.","La turba es vegetación antigua que todavía no ha terminado de convertirse en suelo.")}
  function d7PeatTypesCard(){return d7ImageCard("TRES FORMAS DE TURBERA","komosse-03-tipos-de-turbera-mobile-v3.jpg","Comparación vertical de turbera baja, turbera elevada y turbera de cobertura","Komosse es una turbera elevada: su centro ha crecido y recibe principalmente agua de lluvia.","Lo que cambia es de dónde llega el agua y qué forma toma la turba. Elevada no significa montaña.")}
  function d7PeatTimelineCard(){return d7ImageCard("KOMOSSE · DIEZ MIL AÑOS DE CAMBIOS","komosse-04-cronologia-mobile-v3.jpg","Cronología vertical de diez mil años de Komosse desde el hielo hasta la turbera elevada","El crecimiento no fue una línea recta: hubo etapas más cálidas y secas y, después, un clima más húmedo que permitió volver a acumular turba.","La capa de turba suele medir 3–4 metros en Komosse y, en algunos puntos, supera los 8 metros.")}
  ...
  function d8RocknarCard(){return d8ImageCard("ROCKNAR · ARENA, PINOS Y TURBA", "d8-rocknar-cronologia-mobile-v2.jpg", "Secuencia desde el hielo y el lago de deshielo hasta las dunas boscosas y la turba de Store Mosse", "Hielo, lago, arena, viento, pinos y turba: el paisaje actual reúne tiempos distintos.", "Los rocknar son dunas de arena cubiertas de bosque, no rocas ni torres.")}
  ...
  function d8PaxCard(){
    const scenes=[
      ["d8-pax-01-fundacion-mobile-v2.jpg","1493: Jakob Ulvsson, Kort Rogge y Sten Sture hacen posible la fundación de Pax Mariæ."],
      ["d8-pax-02-paz-maria-mobile-v2.jpg","Pax Mariæ significa Paz de María; era el único monasterio cartujo de Escandinavia."],
      ["d8-pax-03-gustav-mobile-v2.jpg","1523: Gustav Vasa llega al poder y busca reforzar los recursos de la Corona."],
      ["d8-pax-04-reforma-mobile-v2.jpg","1526–1527: la Reforma transforma las relaciones entre la Corona y la Iglesia."],
      ["d8-pax-05-castillo-mobile-v2.jpg","1537: el monasterio se desmantela; su piedra se reutiliza en la obra de Gripsholm."],
      ["d8-pax-06-mariefred-mobile-v2.jpg","1605: la localidad recibe privilegios y conserva el nombre de Mariefred."]
    ];
    return d8Card("PAX MARIÆ → MARIEFRED",`<section class="d8-comic" aria-label="Seis viñetas sobre el monasterio Pax Mariæ y Mariefred">${scenes.map(([file,alt],i)=>`<figure><img src="${imgBase}${file}" alt="${alt}" decoding="sync"><figcaption>VIÑETA ${i+1} DE 6</figcaption></figure>`).join("")}</section>`,"Suecia no dejó de ser cristiana: durante el siglo XVI pasó de un reino católico a uno luterano, mientras la Corona ganó control sobre bienes e ingresos de la Iglesia.")}
  ...
  function d9CrownRoadCard(){const scenes=[
    ["d9-corona-vertical-00-bo-grip-v1.jpg","Mi fortaleza llevará mi nombre.","1379–1404 · Bo Jonsson Grip construye Gripsholm. Tras su muerte, sus herederos la venden a Margarita: entra en la Corona.","Bo Jonsson Grip, sus herederos y la fortaleza medieval de Gripsholm",false,0],
    ["d9-corona-vertical-01-olaf-v1.jpg","Mi hijo ha muerto. La Corona necesita continuidad.","1387 · Muere Olaf, único hijo de Margarita. La Corona necesita un heredero.","Margarita I ante la noticia de la muerte de su hijo Olaf, con Tre Kronor al fondo",false,1],
    ["d9-corona-vertical-02-erik-v1.jpg","¿Una corona para tres reinos?","1397 · Erik es coronado rey de Dinamarca, Noruega y Suecia.","Erik de Pomerania es coronado ante los símbolos de Dinamarca, Noruega y Suecia",true,2],
    ["d9-corona-vertical-03-rivalidades-v1.jpg","Compartir corona no elimina rivalidades.","La Unión de Kalmar dura, pero Suecia discute quién debe gobernarla.","Nobles suecos discuten el poder durante la Unión de Kalmar",false,3],
    ["d9-corona-vertical-04-sture-v1.jpg","Suecia no quiere gobernarse desde lejos.","1520 · Sten Sture el Joven muere tras la batalla de Bogesund.","Sten Sture el Joven, herido tras la batalla de Bogesund, con sus partidarios",true,4],
    ["d9-corona-vertical-05-cristian-v1.jpg","Prometieron amnistía. Las puertas se cierran.","1520 · Cristián II promete amnistía; el Día 1 cuenta lo que ocurrió después.","Cristián II llega a caballo y promete amnistía",false,5],
    ["d9-corona-vertical-06-gustavo-v1.jpg","Suecia elegirá a su propio rey.","1523 · Gustavo Vasa es elegido rey de Suecia.","Gustavo Vasa es aclamado rey de Suecia en 1523",true,6]
  ];return d9Card("UNA CORONA · MÁS DE 125 AÑOS DE TENSIÓN",`<section class="d9-crown-vertical-comic" aria-label="Siete viñetas verticales desde Bo Jonsson Grip hasta Gustavo Vasa">${scenes.map(([file,speech,caption,alt,right,number])=>`<figure class="d9-crown-panel${right?" is-right":""}"><img src="${imgBase}${file}" alt="${alt}" decoding="sync"><span class="d9-crown-speech">«${speech}»</span><figcaption>${caption}</figcaption><small>VIÑETA ${number} DE 6</small></figure>`).join("")}</section>`,"La viñeta 0 explica cómo Gripsholm entra en la Corona. Los diálogos recreados cuentan la historia; las cartelas fijan las fechas y los hechos.")}
  function d9SturesCard(){return d9Card("DOS STEN STURE · NO ERAN FAMILIA",`<div class="d9-sture-compare"><div><b>STEN STURE EL VIEJO</b><span>familia Sture</span><strong>Gripsholm · 1472</strong><small>Obtiene la finca; muere en 1503.</small></div><div><b>STEN SVANTESSON</b><span>hijo de Svante Nilsson · Natt och Dag</span><strong>Regente · 1512</strong><small>Usa «Sture» para aprovechar el prestigio político del Viejo.</small></div></div>`,"En 1512 se le conoce como Sten Sture el Joven. No hereda una familia: adopta un nombre prestigioso. La sucesión política fue Viejo → Svante Nilsson → Joven.")}
  function d9PaxCard(){const scenes=[
    ["d9-pax-vertical-01-corona-v1.jpg","Esta fortaleza pasa a ser propiedad de la Corona.","1404 · Margarita incorpora Gripsholm a la Corona.","Margarita I incorpora la fortaleza medieval de Gripsholm a las tierras de la Corona"],
    ["d9-pax-vertical-02-intercambio-v1.jpg","No es un regalo: cambio tierras heredadas por Näsby y Gripsholm.","1472 · Sten entrega Ekebyhof y otros bienes; recibe Näsby, la finca donde está Gripsholm.","Sten Sture el Viejo negocia un intercambio de tierras por Näsby y Gripsholm"],
    ["d9-pax-vertical-03-monasterio-v1.jpg","Queremos una cartuja: estudio, oración y silencio.","1493 · Los obispos impulsan el monasterio y Sten Sture aporta Gripsholm. De Pax Mariæ nace Mariefred.","Los obispos y Sten Sture planean el monasterio cartujo Pax Mariæ junto al lago Mälaren"],
    ["d9-pax-vertical-04-recupera-v1.jpg","La Corona reclama la herencia de los Sture.","1526 · Gustavo Vasa disuelve Pax Mariæ y toma Gripsholm. Es el cambio concreto de propiedad.","Gustavo Vasa y sus escribanos inventarían los bienes del monasterio tras la Reforma"],
    ["d9-pax-vertical-05-castillo-v1.jpg","Aquí habrá una residencia fortificada.","1537 · Gustavo Vasa derriba gran parte del antiguo monasterio y comienza el castillo de ladrillo que habéis visitado.","Gustavo Vasa supervisa la construcción del actual castillo de ladrillo de Gripsholm"]
  ];return d9Card("UNA LLAVE · CINCO MANOS",`<section class="d9-pax-vertical-comic" aria-label="Cinco viñetas verticales sobre los cambios de propiedad de Gripsholm">${scenes.map(([file,speech,caption,alt],i)=>`<figure class="d9-pax-panel"><img src="${imgBase}${file}" alt="${alt}" decoding="sync"><span class="d9-pax-speech">«${speech}»</span><figcaption>${caption}</figcaption><small>VIÑETA ${i+1} DE 5</small></figure>`).join("")}</section><div class="d9-word-chain"><span><b>PAX MARIÆ</b><small>latín · Paz de María</small></span><i>→</i><span><b>MARIAS FRED</b><small>sueco · Paz de María</small></span><i>→</i><span><b>MARIEFRED</b><small>nombre de la ciudad</small></span></div>`,"Diálogos recreados para contar la historia; las cartelas fijan fechas y cambios de propiedad.")}
  ...
  function d9CastlesCard(){return d9Card("SEIS LUGARES · SEIS FORMAS DE PODER",`<div class="d9-sweden-map"><svg viewBox="0 0 320 450" role="img" aria-label="Mapa esquemático de Suecia meridional y central con seis lugares históricos"><path d="M166 20c30 13 44 42 34 69l8 34-17 31 13 35-21 35 6 41-22 42 5 44-26 28 5 48-33 26-32-18-12-44 14-37-10-42 18-37-3-44 19-28-5-42 24-30-3-34 30-28z" fill="#dce9e6" stroke="#1678a7" stroke-width="3"/><path d="M50 346c34-23 58-20 75 2l-10 54-50-5z" fill="#dce9e6" stroke="#1678a7" stroke-width="3"/><path d="M50 384c42 13 73 16 111-1" fill="none" stroke="#8fb9c8" stroke-width="5"/><g class="d9-map-dot"><circle cx="151" cy="356" r="13"/><text x="151" y="361">1</text><circle cx="169" cy="230" r="13"/><text x="169" y="235">2</text><circle cx="109" cy="233" r="13"/><text x="109" y="238">3</text><circle cx="91" cy="274" r="13"/><text x="91" y="279">4</text><circle cx="65" cy="325" r="13"/><text x="65" y="330">5</text><circle cx="140" cy="248" r="13"/><text x="140" y="253">6</text></g><text x="168" y="375" class="d9-map-label">KALMAR</text><text x="186" y="225" class="d9-map-label">ESTOCOLMO</text><text x="26" y="229" class="d9-map-label">ÖREBRO</text><text x="10" y="292" class="d9-map-label">KARLSBORG</text><text x="75" y="348" class="d9-map-label">GOTEMBURGO</text><text x="147" y="263" class="d9-map-label">GRIPSHOLM</text></svg></div><p class="d9-map-note">Mapa de situación: los puntos respetan la posición relativa de los seis lugares.</p><div class="d9-place-list d9-place-photo-list"><article><figure><img src="${imgBase}d9-kalmar-castle-cc-by-sa.jpg" alt="Castillo de Kalmar junto al mar"><figcaption>Foto: W. Bulach · <a href="https://commons.wikimedia.org/wiki/File:00_2070_Kalmar_Castle,_Sweden.jpg" target="_blank" rel="noopener">CC BY-SA 4.0</a></figcaption></figure><div><b><i>1</i> KALMAR · UNA CORONA PARA TRES REINOS</b><span>En 1397, aquí se unieron Dinamarca, Noruega y Suecia bajo una misma corona. Es el comienzo de la historia de Margarita, Erik de Pomerania y la unión que acabará rompiéndose.</span></div></article><article><figure><img src="${imgBase}d9-stockholm-royal-palace-cc-by-sa.jpg" alt="Palacio Real de Estocolmo en el lugar donde estuvo Tre Kronor"><figcaption>Foto: Suicasmo · <a href="https://commons.wikimedia.org/wiki/File:Exterior_of_the_Royal_Palace,_Stockholm_20180731-2.jpg" target="_blank" rel="noopener">CC BY-SA 4.0</a></figcaption></figure><div><b><i>2</i> ESTOCOLMO · DONDE SE DECIDÍA EL PODER</b><span>Aquí estuvo Tre Kronor, el castillo desde el que se gobernaba Suecia. En este lugar encajan Cristián II, la amnistía prometida y el Baño de Sangre del Día 1.</span><small>Foto actual: Palacio Real. Tre Kronor ardió en 1697.</small></div></article><article><figure><img src="${imgBase}orebro-castle-cc0.jpg" alt="Castillo de Örebro rodeado por el río Svartån"><figcaption>Foto del castillo · CC0</figcaption></figure><div><b><i>3</i> ÖREBRO · EL CASTILLO DE LA REBELIÓN</b><span>En 1434 Engelbrekt tomó esta fortaleza. Örebro era cruce de río, caminos y comercio: quien controlaba este castillo podía controlar el paso y desafiar al rey.</span></div></article><article><figure><img src="${imgBase}d9-karlsborg-fortress-cc-by.jpg" alt="Vista aérea de la fortaleza de Karlsborg"><figcaption>Foto: Jan Norrman · <a href="https://commons.wikimedia.org/wiki/File:Karlsborgs_f%C3%A4stning_-_Flygfoto.jpg" target="_blank" rel="noopener">CC BY 2.5 SE</a></figcaption></figure><div><b><i>4</i> KARLSBORG · EL PLAN B DE SUECIA</b><span>Era el refugio preparado para el rey, el gobierno y los tesoros del país si Suecia era atacada. Conecta con el Día 5: rutas, canal y defensa.</span></div></article><article><figure><img src="${imgBase}skansen-kronan-public-domain.jpg" alt="Fortaleza Skansen Kronan en Gotemburgo"><figcaption>Foto de apoyo · dominio público</figcaption></figure><div><b><i>5</i> SKANSEN KRONAN · DEFENDER GOTEMBURGO</b><span>La construyeron con 23 cañones para proteger la ciudad. Nunca fue atacada; después fue prisión, vivienda y museo. Un mismo edificio puede tener muchas vidas.</span></div></article><article><figure><img src="${imgBase}d9-gripsholm-castle-cc-by-sa.jpg" alt="Castillo de Gripsholm en Mariefred"><figcaption>Foto: Hartmut Schmidt Heidelberg · <a href="https://commons.wikimedia.org/wiki/File:Castle_of_Gripsholm,_Stockholm,_Sweden.jpg" target="_blank" rel="noopener">CC BY-SA 4.0</a></figcaption></figure><div><b><i>6</i> GRIPSHOLM · LA LLAVE FINAL</b><span>Primero fue finca noble, después tierra del monasterio de Mariefred y, con Gustavo Vasa, castillo real. También fue prisión y museo: aquí se unen los días 8 y 9.</span></div></article></div>`,"No son seis castillos iguales: son seis lugares que responden a preguntas distintas sobre poder, rutas y defensa.")}
  ...
  function d9FamilyCard(){return d9Card("FAMILIA VASA · BODAS, HIJOS Y LLAVES",`<section class="d9-vasa-tree" aria-label="Árbol familiar de Gustavo Vasa, Erik XIV y Johan III"><div class="d9-vasa-root"><b>👑 GUSTAVO VASA</b><span>rey desde 1523</span></div><p class="d9-vasa-root-note">Tres matrimonios; dos ramas que acabarán peleando por la corona.</p><div class="d9-vasa-marriages"><article><time>1531–1535</time><div><b>Gustavo Vasa</b><i>✦</i><b>Katarina de Sajonia-Lauenburg</b></div><p><strong>Hijo:</strong> Erik XIV</p></article><article class="d9-vasa-main-marriage"><time>1536–1551</time><div><b>Gustavo Vasa</b><i>✦</i><b>Margarita Leijonhufvud</b></div><p><strong>Hijos:</strong> Johan III y nueve más, entre ellos Karl IX.</p></article><article><time>1552–1560</time><div><b>Gustavo Vasa</b><i>✦</i><b>Katarina Stenbock</b></div><p><strong>Hijos:</strong> ninguno.</p></article></div><p class="d9-vasa-connector">ERIK XIV Y JOHAN III · MEDIOS HERMANOS</p><div class="d9-vasa-branches"><article><header><b>👑 ERIK XIV</b><span>hijo de Katarina de Sajonia-Lauenburg</span></header><div class="d9-vasa-union"><b>1568</b><span>se casa con</span><strong>KARIN MÅNSDOTTER</strong></div><p><strong>Hijos:</strong> Sigrid, Gustavo, Enrique y Arnoldo.</p><footer>🔒 <b>Gripsholm, 1571–1573</b><span>Johan encierra aquí a Erik, Karin y los niños.</span></footer></article><article><header><b>👑 JOHAN III</b><span>hijo de Margarita Leijonhufvud</span></header><div class="d9-vasa-union"><b>1562</b><span>se casa con</span><strong>KATARINA JAGELLONICA</strong></div><p><strong>Hijos:</strong> Isabella, Segismundo y Ana.<br><small>Isabella y Segismundo nacen en Gripsholm.</small></p><footer>🔒 <b>Gripsholm, 1563–1567</b><span>Erik encierra aquí a Johan, Katarina y sus hijos.</span></footer></article></div><div class="d9-vasa-switch"><b>1563–67</b><span>Erik encierra a Johan</span><i>↓</i><b>1568</b><span>Johan toma la corona</span><i>↓</i><b>1571–73</b><span>Johan encierra a Erik</span></div></section>`,"👑 = llegó a reinar · 🔒 = estuvo preso o presa en Gripsholm. El árbol sigue los matrimonios y los hijos; la franja final muestra cómo cambiaron las llaves del castillo.")}
  ...
  function d9LionCard(){return d9Card("DOS IMÁGENES QUE ENGAÑAN UN POCO",`<figure class="d9-story-art"><img src="${imgBase}d9-leon-casco-comparativa-ilustracion-v1.png" alt="Ilustración comparativa del león de Gripsholm y un casco vikingo con cuernos"><figcaption>Ilustración explicativa · no sustituye a los objetos reales.</figcaption></figure><div class="d9-lion-facts"><div><b>LEÓN DE GRIPSHOLM</b><span>La taxidermia parece extraña porque probablemente su autor nunca había visto un león vivo.</span></div><div><b>CASCO CON CUERNOS</b><span>La imagen moderna se popularizó en ilustraciones y teatro; no describe un casco vikingo de combate.</span></div></div>`,"No digas «es falso y ya está». Pregunta: ¿quién hizo esta imagen?, ¿qué conocía? y ¿qué nos enseña hoy?")}
  ...
  render();
  function mapPlaceCards(kind,places){return `<div class="map-place-cards ${kind}-place-cards" aria-label="Paradas del mapa">${places.map((p,i)=>`<button type="button" class="map-place-card" data-${kind}-place="${p.id}"><span class="map-place-number">${i+1}</span><figure><img src="${p.imgUrl||imgBase+p.img}" alt="${p.n}" style="object-fit:${p.fit||'cover'};object-position:${p.pos||'center'}"><figcaption>${p.credit}</figcaption></figure><span class="map-place-copy"><b>${p.n}</b><small>${p.copy}</small></span></button>`).join("")}</div>`}
  ...
  function d4ParkPlaces(){return [
      ...
      {id:"junker",n:"Junker Jägares sten",ll:[58.7300,14.5764],img:"junker-jagare-stone-public-domain.jpg",credit:"Imagen de apoyo del cuaderno",copy:"La roca está a poca distancia de Vitsand por la ruta blanca: alrededor de 600–900 metros según el punto exacto de inicio y parada.",tip:{direction:"right",offset:[17,-18]}},
      {id:"stigman",n:"Stigmanspasset",ll:[58.7160,14.5920],img:"stigmanspasset-2026.jpg",credit:"Imagen aportada para este cuaderno",copy:"La ruta verde atraviesa un paso entre paredes de roca. El relato de salteadores es una leyenda, no una prueba de un asalto.",tip:{direction:"bottom",offset:[0,18]}},
      {id:"trollkyrka",n:"Trollkyrka",ll:[58.7200,14.6098],img:"trolls-tiveden-recreacion-v1.png",credit:"Ilustración narrativa del cuaderno",copy:"Son cumbres rocosas usadas como referencias del paisaje. El nombre pertenece a las leyendas, no demuestra un templo de trolls.",tip:{direction:"right",offset:[17,0]}},
      {id:"stenkallan",n:"Stenkällan",ll:[58.7203,14.5846],img:"stenkallan-tiveden-silverkey.jpg",credit:"Foto: Silverkey (Mickaël Delcey) · CC BY-SA 4.0 · Wikimedia Commons",copy:"Bloques, grietas y cavidades del granito: una parada para conectar la excursión con la historia del hielo del Día 4.",tip:{direction:"left",offset:[-17,-8]}}
    ]}
  function bindD4ParkRoutes(){
    const tracks=[
      {c:"#7650a1",p:[[14.56917,58.714],[14.56549,58.71231],[14.56022,58.71342],[14.55341,58.71493],[14.55278,58.71914],[14.55446,58.72494],[14.55606,58.7293],[14.56138,58.72823],[14.56848,58.72888],[14.57478,58.72551],[14.58215,58.71857],[14.59094,58.71561],[14.58431,58.715],[14.57772,58.71438],[14.57116,58.71654],[14.56917,58.714]]},
      {c:"#183d4c",under:true,p:[[14.56925,58.72877],[14.57203,58.73048],[14.57432,58.73255],[14.57635,58.73342],[14.57832,58.73232],[14.57638,58.73098],[14.57563,58.72902],[14.5756,58.72738],[14.57711,58.72661],[14.57832,58.72557],[14.57619,58.7254],[14.57321,58.72674],[14.56925,58.72877]]},
      {c:"#2f7b68",p:[[14.59195,58.71597],[14.59195,58.71752],[14.59377,58.71941],[14.59477,58.72165],[14.59383,58.72296],[14.58964,58.72322],[14.58583,58.72481],[14.58246,58.7258],[14.57866,58.72585],[14.57602,58.72507],[14.57847,58.72332],[14.58111,58.7217],[14.5847,58.72034],[14.58805,58.71843],[14.59135,58.71599]]},
      {c:"#c44738",p:[[14.59217,58.71565],[14.59715,58.7154],[14.60162,58.71583],[14.60424,58.71763],[14.60666,58.71932],[14.61067,58.72011],[14.61514,58.71856],[14.6136,58.71717],[14.61222,58.71509],[14.61008,58.71528],[14.60813,58.71532],[14.60711,58.71454],[14.60312,58.71444],[14.60008,58.71369],[14.59644,58.7143],[14.59217,58.71565]]}
    ],places=d4ParkPlaces();
    const select=p=>document.querySelectorAll("[data-d4-place]").forEach(card=>card.classList.toggle("active",card.dataset.d4Place===p.id));
    document.querySelectorAll("[data-d4-place]").forEach(card=>card.onclick=()=>select(places.find(p=>p.id===card.dataset.d4Place)));
  ...
  function d3OrebroPlaces(){return [
    {id:"wadkoping",n:"Wadköping",ll:[59.27256,15.23223],img:"Wadköping_i_Örebro.jpg",credit:"Foto de Wadköping · Wikimedia Commons",copy:"Museo al aire libre con casas históricas trasladadas desde Örebro y sus alrededores. Está junto al Svartån, tras el Stadsparken.",tip:{direction:"right",offset:[17,0]}},
    {id:"castle",n:"Castillo de Örebro",ll:[59.27420,15.21480],img:"orebro-castle-cc0.jpg",credit:"Foto del castillo · CC0",copy:"Fortaleza levantada en un islote del Svartån. El agua, el puente y la defensa explican por qué este paso tenía poder.",tip:{direction:"top",offset:[0,-16]}},
    {id:"engelbrekt",n:"Engelbrekt · Stortorget",ll:[59.272135,15.211772],img:"engelbrekt-statue-orebro.jpg",credit:"Foto: Einarspetz · CC BY-SA 3.0 · Wikimedia Commons",copy:"Estatua posterior de Engelbrekt Engelbrektsson en Stortorget, junto a la iglesia de San Nicolás. Sirve para recordar la figura histórica; no es un retrato de su aspecto real.",tip:{direction:"left",offset:[-18,11]}},
    {id:"bernadotte",n:"Karl XIV Johan",ll:[59.27482,15.21449],img:"bernadotte-public-domain.jpg",credit:"Foto histórica de Jean Baptiste Bernadotte · dominio público",copy:"La estatua de Karl XIV Johan está en Henry Allards park. Bernadotte fue elegido heredero de Suecia en la Dieta de Örebro de 1810.",tip:{direction:"right",offset:[20,18]}}
  ]}
  function d3OrebroExplorer(){const places=d3OrebroPlaces();return `<section class="orebro-city-explorer"><div id="d3-orebro-real-map" class="orebro-real-map" role="img" aria-label="Mapa geográfico de Örebro con Wadköping, el castillo y las estatuas de Engelbrekt y Karl XIV Johan"><p>Cargando el mapa real de Örebro…</p></div><p class="map-caption">Mapa urbano real: río Svartån, calles y parques están a escala. Los números llevan el nombre de cada parada; debajo quedan sus cuatro fichas completas.</p>${mapPlaceCards("d3",places)}</section>`}
  ...
  function bindD3OrebroExplorer(){
    const places=d3OrebroPlaces();
    const select=p=>document.querySelectorAll("[data-d3-place]").forEach(card=>card.classList.toggle("active",card.dataset.d3Place===p.id));
    const start=()=>{const el=$("#d3-orebro-real-map");if(!el||el.dataset.ready)return;el.dataset.ready="1";
      ...
      places.forEach((p,i)=>{const icon=L.divIcon({className:"d3-city-number",html:String(i+1),iconSize:[31,31],iconAnchor:[15,15]});const marker=L.marker(p.ll,{icon}).addTo(map).on("click",()=>select(p));marker.bindTooltip(`${i+1} · ${p.n}`,{permanent:true,direction:p.tip.direction,offset:p.tip.offset,className:"map-place-label"});group.addLayer(marker)});
  ...
  function d5HjoPlaces(){return [
    ...
    {id:"icecream",n:"Moster Elins Glasscafé",ll:[58.30269,14.29375],img:"moster-elins-hjo-2026.jpg",credit:"Foto aportada para este cuaderno, 2026",copy:"La heladería está en Hamnvaktstugan, junto al puerto. Es una parada muy conocida para tomar un helado antes o después del paseo.",tip:{direction:"left",offset:[-17,-18]}},
  ...
  function d6HansaPlaces(){return [
    {id:"lodose",n:"Lödöse Museum",ll:[58.029,12.160],imgUrl:"https://www.vastsverige.com/globalassets/migratedresources/lilla-edet/img_9434.jpg?f=webp&h=525&m=Crop&q=75&w=720",credit:"Wikimedia Commons · licencia en archivo",copy:"La salida medieval río arriba: comercio anterior y mundo de la Hansa.",tip:{direction:"right",offset:[18,0]}},
    {id:"statue",n:"Gustaf Adolfs torg",ll:[57.70712,11.96682],imgUrl:d6Img("Gustav II Adolf Göteborg.jpg"),fit:"contain",pos:"center top",credit:"Gegik · CC0 · Wikimedia Commons",copy:"La tradición coloca aquí el gesto del rey; la fundación de 1621 está documentada.",tip:{direction:"left",offset:[-18,0]}},
  ...
  function d6GothenburgPlaces(){return [
    {id:"king",n:"Estatua de Gustavo II Adolfo",ll:[57.70712,11.96682],imgUrl:d6Img("Gustav II Adolf Göteborg.jpg"),fit:"contain",pos:"center top",credit:"Gegik · CC0 · Wikimedia Commons",copy:"El rey fundador señala hacia el suelo: la decisión de levantar la ciudad junto al mar.",tip:{direction:"right",offset:[18,0]}},
    {id:"canal",n:"Stora Hamnkanalen",ll:[57.70665,11.96615],imgUrl:"https://image-worker.mindtrip.ai/image-resize/format%3Dwebp%2Cw%3D1200/https%3A/images.mindtrip.ai/attractions/2cd2/fc96/8945/d752/eec0/24b9/a6e4%20",credit:"Wikimedia Commons · licencia en archivo",copy:"Canal, drenaje y foso: el agua como infraestructura de ciudad.",tip:{direction:"right",offset:[18,0]}},
    {id:"museum",n:"Museo de Gotemburgo",ll:[57.70496,11.96538],imgUrl:"https://cms.goteborg.com/uploads/2020/10/Fasad-Ka%E2%95%A0empebron-2500x1666.jpg",credit:"Wikimedia Commons · licencia en archivo",copy:"La antigua Casa de las Indias Orientales: té, porcelana, China y puerto.",tip:{direction:"left",offset:[-18,0]}},
  ...
  function d2StockholmPlaces(){return [
    ...
    {id:"stortorget",n:"Stortorget",ll:[59.3250,18.0707],imgUrl:d2Img("Stortorget square architecture, Gamla stan, Stockholm, Sweden, Northern Europe.jpg"),credit:"Wikimedia Commons · licencia en archivo",copy:"La plaza del Día 1: ciudad medieval y memoria del Baño de Sangre.",tip:{direction:"left",offset:[-18,0]}},
    ...
    {id:"rune",n:"Piedra rúnica U 53",ll:[59.32552,18.07002],imgUrl:d2Img("Prästgatan rune stone 070407.JPG"),credit:"Wikimedia Commons · licencia en archivo",copy:"Runas: escritura; el texto está en nórdico antiguo y conserva un mensaje familiar.",tip:{direction:"right",offset:[18,0]}},
    ...
    {id:"cityhall",n:"Ayuntamiento de Estocolmo",ll:[59.32750,18.05420],imgUrl:d2Img("Stockholm Sweden Stadshuset-02.jpg"),credit:"Wikimedia Commons · licencia en archivo",copy:"Nobel, Salón Azul, Tres Coronas y torre de 106 metros.",tip:{direction:"left",offset:[-18,0]}},
    {id:"hornsgatan",n:"Hornsgatan · segunda mano",ll:[59.31930,18.05220],imgUrl:d2Img("Hornsgatan mars 2008.jpg"),credit:"Wikimedia Commons · licencia en archivo",copy:"El tramo de Södermalm con tiendas vintage y segunda mano.",tip:{direction:"right",offset:[18,0]}},
    {id:"montelius",n:"Monteliusvägen · atardecer",ll:[59.31810,18.05710],imgUrl:d2Img("Stockholm, view from Monteliusvägen, 2019-07-28.jpg"),credit:"Wikimedia Commons · licencia en archivo",copy:"El final del paseo: agua, Riddarholmen y Ayuntamiento al caer el sol.",tip:{direction:"right",offset:[18,0]}}
  ]}
