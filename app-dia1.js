(() => {
  ...
  function d5GotaExploreMap(q,scope){
    ...
    stops.forEach((x,i)=>{const p=project(x[2],x[3],b,m.W,m.H),active=opened.includes(i);s+=`<g class="map-explore-pin ${active?"opened":""}" data-explore="${i}" role="button" tabindex="0" aria-label="Abrir etapa ${x[0]}: ${x[1]}"><circle cx="${p[0]}" cy="${p[1]}" r="12"/><text x="${p[0]}" y="${p[1]+5}" text-anchor="middle">${active?"✓":x[0]}</text><text class="map-label" x="${p[0]+x[4]}" y="${p[1]+x[5]}">${x[1]}</text></g>`});
    const active=opened.length?opened[opened.length-1]:null,detail=active==null?`<p class="map-explore-detail"><strong>Empieza por el 1.</strong> Cada número abre una etapa de oeste a este.</p>`:`<p class="map-explore-detail"><strong>${active+1}. ${esc(q.items[active][0])}.</strong> ${esc(q.items[active][1])}</p>`;
    return d5Card("SISTEMA GÖTA · SEIS ETAPAS DE COSTA A COSTA",`<div class="geo-map interactive-geo-map">${s}</svg></div><p class="map-caption">Toca los números en orden: Gotemburgo → Mem. El punto se ilumina y abre su explicación.</p>${detail}<div class="map-explore-order">${q.items.map((x,i)=>`<button type="button" class="map-explore-step ${opened.includes(i)?"opened":""}" data-explore="${i}"><b>${opened.includes(i)?"✓":i+1}</b><span>${esc(x[0])}</span></button>`).join("")}</div><p class="placement-progress">Etapas abiertas: ${opened.length} de ${q.items.length}</p>`)
  }
  ...
  function d6DrainCard(){return d6Card("CUANDO LLUEVE, EL AGUA NECESITA CAMINO",`<div class="drain-flow"><div><b>LLUVIA</b><span>calles y tejados recogen agua</span></div><i>↓</i><div><b>ZANJA</b><span>la aleja de los cimientos</span></div><i>↓</i><div><b>CANAL</b><span>lleva el caudal de muchas calles</span></div><i>↓</i><div><b>RÍO → MAR</b><span>salida final hacia el Kattegat</span></div></div><p class="drain-warning"><strong>Si un tramo se bloquea:</strong> el agua se acumula, el barro se reblandece y pueden inundarse calles, almacenes y casas.</p>`,"El mismo esquema aparece de nuevo en el reto: sirve para seguir el agua paso a paso.")}
  ...
  function d6TeaWorldMap(){return d6Card("MAPA MUNDIAL ESQUEMÁTICO · GOTEMBURGO ⇄ CANTÓN",`<div class="tea-world-map" role="img" aria-label="Mapa mundial esquemático con Europa, África y Asia y la ruta de Gotemburgo a Cantón por el Atlántico, el cabo de Buena Esperanza y el océano Índico"><div class="world-europe">EUROPA<br><b>Gotemburgo</b></div><div class="world-africa">ÁFRICA<br><small>cabo de Buena Esperanza</small></div><div class="world-asia">ASIA<br><b>Cantón</b></div><div class="world-ocean atlantic">ATLÁNTICO</div><div class="world-ocean indian">OCÉANO ÍNDICO</div><svg viewBox="0 0 100 100" aria-hidden="true"><path d="M27 20 C20 32,35 37,39 64 S55 75,70 57 S79 40,83 27"/></svg><span class="route-dot start">1</span><span class="route-dot cape">2</span><span class="route-dot end">3</span></div><div class="route-legend"><span><b>1</b> Gotemburgo · mar del Norte</span><span><b>2</b> África meridional · cabo de Buena Esperanza</span><span><b>3</b> Cantón · mar de China Meridional</span></div>`,"Ruta completa y esquemática: Europa, África y Asia se ven a la vez. No está a escala; los viajes reales dependían de vientos, escalas y navegación.")}
  ...
  function d7GotarMap(){return d7Card("MAPA DE SUECIA · TRES GRANDES REGIONES",`<svg class="sweden-regions-svg" viewBox="0 0 360 430" role="img" aria-label="Mapa esquemático de Suecia dividido en Norrland, Svealand y Götaland; Västergötland aparece marcado dentro de Götaland"><defs><clipPath id="sweden-shape"><path d="M158 12 L205 23 229 54 218 80 244 109 233 137 252 165 238 194 260 221 241 250 256 278 237 304 245 333 222 361 190 397 157 416 123 397 110 367 85 342 94 313 79 284 94 253 83 224 99 195 89 164 110 137 102 106 124 81 117 51 Z"/></clipPath></defs><rect width="360" height="430" rx="18" fill="#b9dde7"/><g clip-path="url(#sweden-shape)"><rect x="60" y="0" width="220" height="170" fill="#56819a"/><rect x="60" y="170" width="220" height="100" fill="#7e9d6e"/><rect x="60" y="270" width="220" height="170" fill="#ac7b49"/></g><path d="M158 12 L205 23 229 54 218 80 244 109 233 137 252 165 238 194 260 221 241 250 256 278 237 304 245 333 222 361 190 397 157 416 123 397 110 367 85 342 94 313 79 284 94 253 83 224 99 195 89 164 110 137 102 106 124 81 117 51 Z" fill="none" stroke="#274f5b" stroke-width="4"/><text x="170" y="100" text-anchor="middle">NORRLAND</text><text x="170" y="220" text-anchor="middle">SVEALAND</text><text x="170" y="330" text-anchor="middle">GÖTALAND</text><circle cx="130" cy="332" r="8" fill="#f5c500" stroke="#082b3b" stroke-width="3"/><path d="M138 332 L265 332" stroke="#082b3b" stroke-width="2"/><text class="small" x="272" y="328">Västergötland</text><text class="tiny" x="272" y="344">Ulricehamn · Torpa</text><text class="sea" x="320" y="190" transform="rotate(90 320 190)">MAR BÁLTICO</text></svg>`,"Norrland, Svealand y Götaland son las tres grandes regiones históricas. El punto sitúa Västergötland dentro de Götaland.")}
