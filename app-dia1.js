(() => {
  ...
  function optionalQuiz(){
    ...
    if(!item){screen.innerHTML=`<p class="key-count">RETO OPCIONAL COMPLETADO</p><h2>${q.title}</h2><div class="feedback"><strong>SEÑALES RECUPERADAS</strong><p>${q.explanation}</p></div>`;return setActions([{label:"Volver a empezar",kind:"secondary",run:()=>{delete state.progress["optional-truefalse"];save();render()}},{label:"Volver al menú",run:()=>{state.stage="landing";save();render()}}])}
    ...
    if(!answered){screen.querySelectorAll("[data-optional-bool]").forEach(b=>b.onclick=()=>{p.answer=b.dataset.optionalBool==="true";save();render()});setActions([{label:"Elige una opción",disabled:true,run:()=>{}}]);}
    else setActions([{label:i===q.items.length-1?"Guardar el reto opcional":"Siguiente señal",run:()=>{p.index=i+1;delete p.answer;save();render()}},{label:"Volver al menú",kind:"secondary",run:()=>{state.stage="landing";save();render()}}]);
  }
  ...
  function bindExplore(q,scope,correction){
    ...
    setActions([{label:"Guardar las pistas",disabled:p.opened.length!==q.items.length,run:()=>{markAttempt(q,scope,true,correction);complete(q,scope,correction)}}]);
  }
  ...
  function nextLabel(){const m=missions[state.mission];return state.key===m.keys-1?(m.epilogue?"Escuchar a Karin":"Completar la misión"):"Ir a la siguiente clave"}
  ...
  function missionStory(){
    ...
    setActions([{label:state.storyPage===m.epilogue.length-1?"Conservar la decisión":"Continuar",disabled:!!p.options&&state.choices[p.choiceId]==null,run:()=>{if(state.storyPage<m.epilogue.length-1)state.storyPage++;else state.stage="missionEnd";save();render()}}],true);
  }
  function missionEnd(){
  ...
  function d3RouteMap(){return d3MapCard("ESTOCOLMO → ÖREBRO → TIVEDEN",annotatedMap([10,20,56,61.5],[["Estocolmo",18.07,59.33],["Mälaren",16.6,59.5],["Örebro",15.21,59.27],["Hjälmaren",15.9,59.25],["Tiveden",14.65,58.75],["Vänern",13.2,58.9],["Vättern",14.5,58.8]],[[18.07,59.33,15.21,59.27,"ruta actual",""],[15.21,59.27,14.65,58.75,"hacia el bosque","danger"]],"Ruta de Estocolmo a Örebro y Tiveden"),"La jornada pasa de la salida marítima de Estocolmo a un cruce del interior y termina en un bosque entre grandes lagos.")}
  function d3CrossroadsMap(){return d3MapCard("CUATRO CAMINOS · UN PUENTE",`<div class="evidence-grid two"><div><b>NORTE</b><span>Bergslagen y Dalarna · hierro, bosques, noticias</span></div><div><b>ESTE</b><span>Mälaren, Estocolmo y Báltico · mercados y mar</span></div><div><b>SUR</b><span>Östergötland · campos y otros centros urbanos</span></div><div><b>NIDAROS</b><span>dirección de rutas largas hacia Trondheim</span></div><div><b>ÖREBRO</b><span>banco de grava + puente + Svartån</span></div><div><b>CASTILLO</b><span>paso, defensa, impuestos y comercio</span></div></div>`,"El mapa no dibuja carreteras modernas: explica por qué un puente podía conectar regiones enteras.")}
  function d3BalticMap(){return d3MapCard("BÁLTICO · KALMAR · BERGSLAGEN",annotatedMap([5,32,50,67],[["Pomerania",15.5,54.2],["Lübeck / Hansa",10.7,53.9],["Holstein",9.8,54.1],["Schleswig",9.5,54.8],["Dinamarca",10.2,56],["Suecia",17,61],["Bergslagen",15.3,60],["Estocolmo",18.07,59.33],["Kalmar",16.36,56.66],["Gotland",18.5,57.5],["Öland",16.65,56.75]],[[15.3,60,10.7,53.9,"hierro y cobre",""],[10.7,53.9,15.3,60,"sal y otros bienes","danger"]],"Mapa del Báltico, la Hansa, Holstein y Bergslagen"),"Las flechas muestran flujos comerciales simplificados: la Hansa era una red de ciudades y mercaderes, no un país único.")}
  function d3JensMap(){return d3MapCard("BERGSLAGEN → VÄSTERÅS",annotatedMap([13,18.7,58.5,61.5],[["Bergslagen",15.3,60.1],["Västerås",16.55,59.61],["Örebro",15.21,59.27],["Estocolmo",18.07,59.33]],[[15.3,60.1,16.55,59.61,"quejas y administración","danger"]],"Mapa de Bergslagen, Västerås y Örebro"),"Jösse/Jens Eriksson era bailío real en Västerås: recaudaba impuestos y aplicaba órdenes de la Corona.")}
  function d3RevoltMap(){return d3MapCard("REBELIÓN DE 1434–1435",annotatedMap([11,21,57,62.5],[["Bergslagen",15.3,60.1],["Dalarna",14.5,61.1],["Borganäs",15.3,60.3],["Västerås",16.55,59.61],["Örebro",15.21,59.27],["Arboga",15.84,59.39]],[[15.3,60.1,14.5,61.1,"organización",""],[15.3,60.1,16.55,59.61,"presión sobre fortalezas","danger"],[16.55,59.61,15.21,59.27,"expansión",""],[15.21,59.27,15.84,59.39,"1435","danger"]],"Expansión de la rebelión de Engelbrekt"),"La protesta comenzó con problemas concretos y se extendió por fortalezas y ciudades. El mapa no afirma que toda persona pensara igual.")}
  function d3LastJourneyMap(){return d3MapCard("ÖREBRO → HJÄLMAREN → ENGELBREKTSHOLMEN",annotatedMap([14,19,58.8,60],[["Örebro",15.21,59.27],["Hjälmaren",15.9,59.25],["Engelbrektsholmen",15.87,59.2],["Göksholm",15.88,59.19],["Estocolmo",18.07,59.33]],[[15.21,59.27,15.87,59.2,"trayecto documentado",""],[15.87,59.2,18.07,59.33,"reunión prevista · ruta exacta incierta","danger"]],"Último viaje de Engelbrekt"),"Línea continua: lugares documentados. La parte posterior hacia Estocolmo señala el objetivo político, no cada paso exacto del viaje.")}
  ...
  function d4TivedenMap(){return d4Card("TIVEDEN · ENTRE VÄNERN Y VÄTTERN",annotatedMap([12,17,57.7,59.7],[["Örebro",15.21,59.27],["Tiveden",14.65,58.75],["Vänern",13.2,58.9],["Vättern",14.5,58.8],["Vitsand",14.58,58.77],["Stenkälla",14.59,58.82],["Junker Jägares sten",14.58,58.78],["Trollkyrka",14.61,58.80]],[[15.21,59.27,14.65,58.75,"llegada desde Örebro","danger"]],"Mapa de Tiveden entre los lagos Vänern y Vättern"),"Mapa de orientación: los puntos del parque se confirman con el mapa oficial y la señalización actual.")}
  function d4GeologyCard(){return d4Card("TIEMPO PROFUNDO",`<div class="history-timeline"><div><strong>hace ~1.000 millones de años</strong><span>la roca se fractura</span></div><div><strong>hasta hace ~10.000 años</strong><span>el hielo aprovecha grietas, pule y transporta</span></div><div><strong>hoy</strong><span>bloques, crestas, lagos y cavidades</span></div></div>`,"El hielo no inventó la roca: trabajó sobre un terreno que ya tenía una historia larguísima.")}
  ...
  function d4ForestCard(){return d4Card("BOSQUE ANTIGUO · HISTORIA REAL",`<div class="evidence-grid three"><div><b>1371–1853</b><span>al menos 86 incendios registrados en la zona estudiada</span></div><div><b>ANILLOS Y CICATRICES</b><span>un árbol puede guardar un fuego antiguo</span></div><div><b>MADERA MUERTA</b><span>hábitat para hongos e insectos</span></div></div>`,"Antiguo no significa aislado de las personas ni de todos los cambios.")}
  ...
  function d4TrollCard(){return `<div class="visual-evidence"><figure class="vasa-photo"><img src="${imgBase}trolls-tiveden-recreacion-v1.png" alt="Ilustración contemporánea de trolls en un bosque"><figcaption>Recreación contemporánea de una leyenda · no es evidencia histórica</figcaption></figure><div class="evidence-grid two"><div><b>STIGMANSPASSET</b><span>paso estrecho asociado a relatos de salteadores</span></div><div><b>TROLLKYRKA</b><span>nombre y relatos; no prueba de un templo ni de trolls reales</span></div></div></div>`}
  ...
  function d4AnimalsCard(){return d4Card("GUÍA DE PISTAS · NO PROMESA DE AVISTAMIENTO",`<div class="evidence-grid two"><div><b>ALCE</b><span>huella grande de pezuña hendida</span></div><div><b>CORZO</b><span>huella de pezuña hendida más pequeña</span></div><div><b>UROGALLO</b><span>ave ligada a bosques; una pluma no basta para identificarla</span></div><div><b>LINCE EUROASIÁTICO</b><span>vive en Suecia, pero es muy esquivo</span></div></div>`,"Observar es dejar distancia: no se sale del sendero ni se sigue a un animal.")}
  function d4LynxCard(){return d4Card("DOS LINCES · DOS ESPECIES",`<div class="evidence-grid two"><div><b>LINCE EUROASIÁTICO</b><span>especie presente en Suecia y gran parte de Eurasia</span></div><div><b>LINCE IBÉRICO</b><span>especie distinta, propia de la península ibérica</span></div></div>`,"Un nombre parecido no significa mismo mapa, hábitat o situación de conservación.")}
  ...
  function d5RouteMap(){return d5Card("TIVEDEN → KARLSBORG → HJO → HÖKERUM",annotatedMap([12,17,57,59.8],[["Tiveden",14.65,58.75],["Vänern",13.2,58.9],["Vättern",14.5,58.8],["Karlsborg",14.52,58.54],["Hjo",14.29,58.30],["Hökerum",13.28,57.88],["Göta Canal",14.46,58.48]],[[14.65,58.75,14.52,58.54,"orilla de Vättern",""],[14.52,58.54,14.29,58.30,"hacia Hjo","danger"],[14.29,58.30,13.28,57.88,"hacia Hökerum",""]],"Mapa de la jornada desde Tiveden a Hökerum"),"Orientación de la jornada: se confirma el trayecto con un mapa actual y la señalización.")}
  function d5LakesCard(){return d5Card("CUATRO GRANDES LAGOS",`${annotatedMap([10,20,57,61],[['Vänern',13.2,58.9],['Vättern',14.5,58.35],['Hjälmaren',15.9,59.25],['Mälaren',17.2,59.4],['Örebro',15.21,59.27],['Estocolmo',18.07,59.33],['Karlsborg',14.52,58.54],['Hjo',14.29,58.30]],[],'Mapa de los cuatro grandes lagos suecos')}<div class="history-timeline"><div><strong>Vänern</strong><span>el mayor lago de Suecia</span></div><div><strong>Vättern</strong><span>el del día</span></div><div><strong>Mälaren</strong><span>entrada histórica hacia Estocolmo y Gripsholm</span></div><div><strong>Hjälmaren</strong><span>cerca de Örebro y del último viaje de Engelbrekt</span></div></div>`,"Un lago no es solo paisaje: en distintas épocas puede ser frontera, ruta, pesca, comercio o defensa.")}
  ...
  function d5GotaMap(){return d5Card("SISTEMA GÖTA · COSTA A COSTA",annotatedMap([10,20,57,59.5],[["Sjötorp",13.98,58.84],["Vänern",13.2,58.9],["Karlsborg / Vättern",14.52,58.54],["Motala",15.04,58.54],["Söderköping",16.32,58.48],["Mem",16.4,58.48],["Báltico",18,58.5]],[[13.98,58.84,14.52,58.54,"lago + canal",""],[14.52,58.54,15.04,58.54,"Vättern","danger"],[15.04,58.54,16.4,58.48,"ríos + canal + esclusas",""]],"Mapa orientativo del sistema Göta"),"190 km en conjunto · 87 km excavados · 58 esclusas. El recorrido combina agua natural y tramos construidos.")}
  ...
  function d5LakeLifeCard(){return d5Card("UN LAGO TIENE «PISOS»",`<div class="evidence-grid two"><div><b>LUCIO</b><span>cazador de orillas y vegetación</span></div><div><b>PERCA</b><span>zonas litorales y aguas intermedias</span></div><div><b>SALMÓN DE LAGO</b><span>agua abierta</span></div><div><b>SALVELINO ÁRTICO</b><span>agua profunda, fría y bien oxigenada</span></div></div>`,"Los ambientes cambian dentro de un mismo lago; observar no exige buscar ni molestar animales.")}
  function d5HjoMap(){return d5Card("HJO · PUERTO, RÍO, PLAZA Y PARQUE",annotatedMap([14.2,14.4,58.25,58.35],[["Puerto",14.285,58.302],["Hjoån",14.295,58.304],["Plaza",14.29,58.303],["Stadsparken",14.28,58.30],["Vättern",14.24,58.30]],[],"Mapa de orientación de Hjo"),"Una capa propia de orientación sobre mapa geográfico. El recorrido real se confirma en el lugar.")}
  function d5CultureCard(){return d5Card("HJO · BARCO, DISEÑO Y OFICIO",`<div class="evidence-grid two"><div><b>S/S TRAFIK · 1892</b><span>recuerda puertos, personas y mercancías en Vättern</span></div><div><b>ESTRID ERICSON · 1924</b><span>funda Svenskt Tenn en Estocolmo</span></div><div><b>ARTESANÍA</b><span>formación, empresa y colaboración con Josef Frank</span></div><div><b>HÖKERUM</b><span>la marca textil apunta hacia el interior</span></div></div>`,"La siguiente pista no está en una cámara acorazada: sigue una red de tejidos y oficios.")}
  ...
  function d6WestMap(){return d6Card("VÄNERN · GÖTA ÄLV · KATTEGAT",annotatedMap([10.4,15.4,56.8,59.8],[["Vänern",13.2,58.9],["Trollhättan",12.29,58.28],["Gotemburgo",11.97,57.71],["Kattegat",11.15,57.55],["Hökerum",13.28,57.84]],[[13.2,58.9,12.29,58.28,"Göta älv",""],[12.29,58.28,11.97,57.71,"hacia el puerto","danger"],[11.97,57.71,11.15,57.55,"salida al mar",""]],"Mapa del Göta älv y Gotemburgo"),"El río conecta el interior y el mayor lago sueco con la costa occidental.")}
  function d6CoastMap(){return d6Card("COSTA OCCIDENTAL · HACIA 1621",annotatedMap([9.5,16.5,56.4,59.8],[["Bohuslän · Noruega–Dinamarca",11.1,58.7],["Halland · Dinamarca",12.4,56.9],["corredor sueco",12.2,57.7],["Gotemburgo",11.97,57.71],["Göta älv",12.0,57.8],["Kattegat",10.5,57.2]],[],"Mapa de la costa occidental en torno a 1621"),"Mapa histórico orientativo: Suecia poseía una franja occidental estrecha; las fronteras actuales no representan la situación de 1621.")}
  ...
  function d6DrainCard(){return d6Card("CÓMO SALE EL AGUA",`<div class="history-timeline"><div><strong>LLUVIA + SUELO HÚMEDO</strong><span>el agua necesita una salida</span></div><div><strong>ZANJA</strong><span>recoge el agua de la calle</span></div><div><strong>CANAL</strong><span>la conduce hacia un curso mayor</span></div><div><strong>GÖTA ÄLV → MAR</strong><span>el sistema termina en la desembocadura</span></div></div>`,"Si se corta un tramo, el agua se acumula en vez de desaparecer.")}
  ...
  function d6NordlingenMap(){return d6Card("NÖRDLINGEN · 1634",annotatedMap([4,18,45,53],[["Nördlingen",10.49,48.85],["Monarquía Hispánica",-3.5,40],["Sacro Imperio",10.5,50],["Suecia",17.5,61]],[[17.5,61,10.49,48.85,"fuerzas suecas y aliadas","danger"],[-3.5,40,10.49,48.85,"fuerzas españolas","" ]],"Mapa de Nördlingen y el contexto europeo"),"La victoria de tropas españolas e imperiales sitúa a España dentro de esta guerra, no como espectadora.")}
  ...
  function d6TeaWorldMap(){return d6Card("GOTEMBURGO ⇄ CANTÓN",annotatedMap([-15,125,-40,65],[["Gotemburgo",11.97,57.71],["Cabo de Buena Esperanza",18.5,-34.4],["Cantón",113.27,23.13]],[[11.97,57.71,18.5,-34.4,"Atlántico",""],[18.5,-34.4,113.27,23.13,"océano Índico","danger"]],"Ruta aproximada de Gotemburgo a Cantón"),"Ruta aproximada: los viajes concretos cambiaban con viento, escalas y navegación.")}
  ...
  function d6PortsCard(){return d6Card("PUERTOS · MILLONES DE TONELADAS · 2024",`${annotatedMap([-11,16,36,61],[["Gotemburgo",11.97,57.71],["Róterdam",4.14,51.95],["Amberes-Brujas",4.4,51.26],["Valencia",-0.32,39.44]],[],"Puertos europeos de referencia")}<div class="history-timeline"><div><strong>Róterdam · 435,8</strong><span>millones de toneladas</span></div><div><strong>Amberes-Brujas · 277,7</strong><span>millones de toneladas</span></div><div><strong>Valencia · 80,7</strong><span>millones de toneladas</span></div><div><strong>Gotemburgo · 21,8</strong><span>millones de toneladas</span></div></div>`,"La comparación usa volumen total: no mide belleza, turismo ni importancia histórica. Cifras actualizables.")}
  ...
  function d7RouteMap(){return d7Card("ÅSUNDEN · SKOTTEK · TORPA · KOMOSSE",annotatedMap([12.4,15.1,57.1,58.4],[["Ulricehamn",13.41,57.79],["Åsunden",13.42,57.78],["Skottek",13.42,57.76],["Torpa Stenhus",13.25,57.64],["Komosse",13.52,57.57],["Store Mosse",13.9,57.25]],[[13.41,57.79,13.42,57.76,"orilla",""],[13.42,57.76,13.25,57.64,"alternativa histórica","danger"],[13.25,57.64,13.52,57.57,"hacia Komosse",""]],"Mapa de Åsunden, Ulricehamn, Torpa y Komosse"),"La línea recta tachada de La Sombra no es un camino: cada paisaje exige una regla distinta.")}
  ...
  function d7BattleMap(){return d7Card("19 DE ENERO DE 1520 · ÅSUNDEN",annotatedMap([10,21,55,62],[["Åsunden / Bogesund",13.42,57.79],["Estocolmo",18.07,59.33],["Dinamarca",10.0,56.0],["Suecia",16.5,61]],[[10.7,56.2,13.42,57.79,"ejército de Cristián II","danger"],[13.42,57.79,18.07,59.33,"traslado de Sten Sture",""]],"Mapa de Åsunden y Estocolmo en 1520"),"La batalla se libró sobre el hielo. Esta reconstrucción histórica no es una indicación de seguridad actual.")}
  ...
  function d7GotarMap(){return d7Card("TRES GRANDES REGIONES HISTÓRICAS",annotatedMap([10,25,54.5,67],[["NORRLAND",18,64],["SVEALAND",17,60.2],["GÖTALAND",14.5,57.6],["Västergötland",13.5,58.1],["Östergötland",15.8,58.2],["Göta älv",12.3,57.8],["Göteborg",11.97,57.71]],[],"Mapa de Norrland, Svealand y Götaland"),"Götar → Götaland → Västergötland → Göta älv → Göteborg: relación histórica y lingüística, no frontera tribal exacta.")}
  ...
  function d7FamilyCard(){return d7Card("ÁRBOL VASA · TRES RAMAS",`<div class="history-timeline"><div><strong>Katarina de Sajonia-Lauenburgo</strong><span>primera esposa · madre de Erik XIV</span></div><div><strong>Margareta Leijonhufvud</strong><span>segunda esposa · madre de Johan III y Karl IX</span></div><div><strong>Katarina Stenbock</strong><span>tercera esposa · creció en Torpa · sin hijos con Gustav</span></div><div><strong>Karl IX</strong><span>padre de Gustavo II Adolfo, fundador de Gotemburgo</span></div></div>`,"El árbol sirve para orientarse, no para memorizar una lista de familias.")}
  ...
  function openMenu(){
    const list=$("#mission-menu");list.innerHTML=missions.map((m,i)=>`<button type="button" data-jump="${i}" ${i>state.completedMissions.length?"disabled":""}><span class="${state.completedMissions.includes(i)?"done":""}">${state.completedMissions.includes(i)?"✓":String(i+1).padStart(2,"0")}</span><strong>${m.title}</strong><small>${state.completedMissions.includes(i)?"completada":i===state.mission?"actual":"bloqueada"}</small></button>`).join("")+(DATA.optionalTrueFalse?`<button type="button" data-optional-quiz><span>◈</span><strong>${DATA.optionalTrueFalse.title}</strong><small>opcional · siempre disponible</small></button>`:"");
