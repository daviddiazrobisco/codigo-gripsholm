(() => {
  ...
  function d6TeaWorldMap(){
    const b=[-180,180,-58,82],m=baseMap(b,"Mapa mundial de la ruta de Gotemburgo a Cantón por el cabo de Buena Esperanza");
    const point=(lon,lat)=>project(lon,lat,b,m.W,m.H),route=[[11.97,57.71],[-5,45],[-20,12],[18.47,-34.36],[57,-20],[80,8],[113.26,23.13]],labels=[["AMÉRICA DEL NORTE",-108,53],["AMÉRICA DEL SUR",-61,-18],["EUROPA",14,69],["ÁFRICA",20,7],["ASIA",92,56],["OCEANÍA",140,-25]];
    let s=m.svg;
    for(let i=0;i<route.length-1;i++){const a=point(...route[i]),z=point(...route[i+1]);s+=`<path class="world-route-segment" d="M${a[0]},${a[1]} Q${(a[0]+z[0])/2},${Math.max(8,(a[1]+z[1])/2-16)} ${z[0]},${z[1]}"/>`}
    labels.forEach(x=>{const p=point(x[1],x[2]);s+=`<text class="world-continent" x="${p[0]}" y="${p[1]}">${x[0]}</text>`});
    [["1",11.97,57.71,"Gotemburgo",8,-9],["2",18.47,-34.36,"Cabo de Buena Esperanza",8,17],["3",113.26,23.13,"Cantón",8,-9]].forEach(x=>{const p=point(x[1],x[2]);s+=`<circle class="world-route-stop" cx="${p[0]}" cy="${p[1]}" r="10"/><text class="world-route-number" x="${p[0]}" y="${p[1]+4}" text-anchor="middle">${x[0]}</text><text class="world-route-label" x="${p[0]+x[4]}" y="${p[1]+x[5]}">${x[3]}</text>`});
    return d6Card("MAPA MUNDIAL · GOTEMBURGO → CANTÓN",`<div class="geo-map world-route-geo">${s}</svg></div><div class="route-legend"><span><b>1</b> Mar del Norte y Atlántico</span><span><b>2</b> Giro por el cabo de Buena Esperanza</span><span><b>3</b> Océano Índico y mar de China Meridional</span></div>`,"Se muestran los continentes completos para entender la escala. La línea es esquemática: los viajes reales dependían de vientos, escalas y navegación.")
  }
